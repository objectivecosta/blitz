...
pub trait Logger {
  fn log_traffic(&self, timestamp: i64, from_ip: &str, from_dns: &str, to_ip: &str, to_dns: &str, packet_size: i64, payload_size: i64) -> bool;
}
...
impl Logger for SQLiteLogger {
    fn log_traffic(&self, timestamp: i64, from_ip: &str, from_dns: &str, to_ip: &str, to_dns: &str, packet_size: i64, payload_size: i64) -> bool {
      ...
      let query = format!("INSERT INTO {} VALUES (?, ?, ?, ?, ?, ?, ?);", self.today_table());
